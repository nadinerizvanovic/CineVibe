...
// Configure database
var connectionString = builder.Configuration.GetConnectionString("DefaultConnection") ?? "Data Source=.;Database=CineVibeDb;User Id=sa;Password=QWEasd123!;TrustServerCertificate=True;Trusted_Connection=True;";
...
{
    var dataContext = scope.ServiceProvider.GetRequiredService<CineVibeDbContext>();
    ...
    _ = Task.Run(async () =>  // The underscore tells the compiler we're intentionally ignoring the result
    {
        ...
        {
            MovieService.TrainRecommenderAtStartup(trainingScope.ServiceProvider);
        }
    });
}
