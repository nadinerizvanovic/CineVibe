...
import 'package:cinevibe_desktop/screens/director_details_screen.dart';
...
class _DirectorListScreenState extends State<DirectorListScreen> {
  ...
  SearchResult<Director>? directors;
  int _currentPage = 0;
  int _pageSize = 5;
  final List<int> _pageSizeOptions = [5, 7, 10, 20, 50];

  // Search for directors with ENTER key, not only when button is clicked
  Future<void> _performSearch({int? page, int? pageSize}) async {
    ...
    var directors = await directorProvider.get(filter: filter);
    debugPrint(directors.items?.firstOrNull?.fullName);
    setState(() {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      directorProvider = context.read<DirectorProvider>();
          ...
          customElevatedButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => DirectorDetailsScreen(),
                  settings: const RouteSettings(name: 'DirectorDetailsScreen'),
                ),
              );
            },
            icon: Icons.add,
  ...
  Widget _buildResultView() {
    final isEmpty =
        directors == null || directors!.items == null || directors!.items!.isEmpty;
    final int totalCount = directors?.totalCount ?? 0;
      ...
      child: Column(
        children: [
          BaseTableDesign(
            ...
            width: 1400,
            height: 500,
            columnWidths: [200, 200, 200, 130, 120, 120], // First Name, Last Name, Nationality, Movies Count, Status, Actions
            columns: [
              DataColumn(
                label: Text(
                  "First Name",
                        ...
                        (e) => DataRow(
                          onSelectChanged: (value) {
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) =>
                                    DirectorDetailsScreen(director: e),
                                settings: const RouteSettings(
                                  name: 'DirectorDetailsScreen',
                                ),
                              ),
                            );
                          },
                          cells: [
                            ...
                            DataCell(
                              Center(
                                child: Text(
                                  e.nationality ?? 'Not specified',
                                  style: TextStyle(
                                    fontSize: 14,
                                    fontWeight: FontWeight.w500,
                                    color: e.nationality != null 
                                        ? const Color(0xFF1E293B)
                                        : const Color(0xFF64748B),
                                    fontStyle: e.nationality == null 
                                        ? FontStyle.italic 
                                        : FontStyle.normal,
                                  ),
                                ),
            ...
            emptySubtext: "Try adjusting your search criteria or add a new director to get started.",
          ),
          SizedBox(height: 30),
          BasePagination(
